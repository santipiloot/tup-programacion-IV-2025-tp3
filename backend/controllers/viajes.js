...
// Aca arme el query en el controlador por los parametros, para evitar usar logica en el models
const viajeControlador = {
      obtener: async (req, res) => {
        try {
            ...
            console.error(error);
            res.status(500).json({ success: false, errores:["Error al obtener los viajes del conductor"] })
        }
    }, 
    crear: async (req, res) => {
        try {
            ...
            console.error(error)
            res.status(500).json({ success: false, errores:["Error al crear el viaje"] })
        }
    },
    actualizar: async (req, res) => {
        try {
            ...
            console.error(error);
            res.status(500).json({ success: false, errores:["Error al actualizar el viaje"] })
        }
    },
    eliminar: async (req, res) => {
        try {
            ...
            console.error(error);
            res.status(500).json({ success: false, errores:["Error al eliminar el viaje"] })
        }
    },
